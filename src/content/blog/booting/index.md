---
title: 计算机是如何启动的？
description: 从打开电源到开始操作，计算机的启动是一个非常复杂的过程。
date: 2013-02-16 20:25:47
---
> 文章来源于阮一峰老师的网络日志[计算机是如何启动的？](https://www.ruanyifeng.com/blog/2013/02/booting.html)
...
（4） 第6-8个字节：主分区最后一个扇区的物理位。  
（5） 第9-12字节：该主分区第一个扇区的逻辑地。  
（6） 第13-16字节：主分区的扇区总数。  
``` 
...
计算机先读取扩展分区的第一个扇区，叫做["扩展引导记录"](https://en.wikipedia.org/wiki/Extended_partition)（Extended boot record，缩写为EBR）。它里面也包含一张64字节的分区表，但是最多只有两项（也就是两个逻辑分区）。
